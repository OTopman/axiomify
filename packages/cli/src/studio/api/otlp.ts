 ...
 */
export async function handlePostOtlpTraces(req: IncomingMessage, res: ServerResponse): Promise<void> {
  try {
    ...
    for (const resourceSpan of resourceSpans) {
      ...
      for (const scopeSpan of scopeSpans) {
        ...
        for (const span of spans) {
          const startTimeMs = Number(BigInt(span.startTimeUnixNano || 0) / 1000000n);
          const endTimeMs = Number(BigInt(span.endTimeUnixNano || 0) / 1000000n);
    ...
    sendJson(res, { error: 'Failed to process traces', details: err.message }, 500);
  }
}
 ...
 */
export async function handlePostOtlpMetrics(req: IncomingMessage, res: ServerResponse): Promise<void> {
  // Read and return 200 OK since OTel metrics are parsed but we already collect native metrics.
  try {
    await readBody(req);
    sendJson(res, { success: true });
  } catch (err: any) {
    sendJson(res, { error: 'Failed to process metrics', details: err.message }, 500);
  }
}
 ...
 */
export async function handlePostOtlpLogs(req: IncomingMessage, res: ServerResponse): Promise<void> {
  try {
    const bodyStr = await readBody(req);
    if (!bodyStr) {
      sendJson(res, { success: true });
      ...
      for (const scopeLog of scopeLogs) {
        ...
        for (const record of logRecords) {
          const timestamp = record.timeUnixNano
            ? new Date(Number(BigInt(record.timeUnixNano) / 1000000n)).toISOString()
          ...
          let level: any = 'info';
          if (['warn', 'warning'].some(l => severityText.includes(l))) level = 'warn';
          else if (['error', 'fatal', 'crit'].some(l => severityText.includes(l))) level = 'error';
          else if (['debug', 'trace'].some(l => severityText.includes(l))) level = 'debug';
          ...
          recordedLogs.push({
            ...
            source: attributes['code.filepath'] 
              ...
              : 'otel-logger',
            isInternal: false,
            requestId: attributes['request_id'] || attributes['requestId'] || undefined,
    ...
    sendJson(res, { error: 'Failed to process logs', details: err.message }, 500);
  }
}
 ...
 */
export function handleGetOtlpTraces(_req: IncomingMessage, res: ServerResponse): void {
  ...
  for (const [traceId, spans] of tracesMap.entries()) {
    ...
    const endTimeMs = Math.max(...spans.map(s => s.startTimeMs + s.durationMs));
    const durationMs = Math.max(0, endTimeMs - startTimeMs);
 ...
 */
export function handleDeleteOtlpTraces(_req: IncomingMessage, res: ServerResponse): void {
