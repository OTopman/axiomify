...
export class TsValidatorEmitter {
  ...
  private emitValidator(emitter: Emitter, type: IRType): void {
    switch (type.kind) {
      ...
      case 'enum':
        const valList = type.values
          .map((v: any) =>
            typeof v.value === 'string' ? `"${v.value}"` : v.value,
          )
          .join(', ');
        emitter.line(
          `export const ${type.id}Schema = z.union([${type.values.map((v: any) => (typeof v.value === 'string' ? `z.literal("${v.value}")` : `z.literal(${v.value})`)).join(', ')}]);`,
        );
        break;

      case 'union':
        emitter.line(
          `export const ${type.id}Schema = z.union([${type.members.map((m: any) => this.renderZodChain(m)).join(', ')}]);`,
        );
        break;

      case 'intersection':
        emitter.line(
          `export const ${type.id}Schema = z.intersection(${type.members.map((m: any) => this.renderZodChain(m)).join(', ')});`,
        );
        break;
