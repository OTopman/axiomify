...
export class TsReactQueryEmitter {
  ...
  private emitHook(emitter: Emitter, ep: IREndpoint): void {
    ...
    if (isMutation) {
      ...
      const argsSignature = reqType
        ? `request: ${reqType}, options?: Omit<UseQueryOptions<${resType}, Error>, 'queryKey' | 'queryFn'>`
        : `options?: Omit<UseQueryOptions<${resType}, Error>, 'queryKey' | 'queryFn'>`;
