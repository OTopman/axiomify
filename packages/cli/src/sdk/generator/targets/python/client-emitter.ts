...
export class PythonClientEmitter {
  ...
  private emitMethod(emitter: Emitter, ep: IREndpoint): void {
    ...
    let args = `self`;
    for (const p of ep.pathParams) args += `, ${this.toSnakeCase(p.name)}: ${this.renderTypeRef(p.type)}`;
    ...
    emitter.block(`async def ${methodName}(${args}) -> ${returnType}:`, ``, () => {
      ...
      for (const p of ep.pathParams) {
        pathExpr = pathExpr.replace(`{${p.name}}`, `{${this.toSnakeCase(p.name)}}`);
      }
      ...
      if (ep.requestBody) {
        ...
        emitter.line(`req_body = body.model_dump(by_alias=True) if hasattr(body, 'model_dump') else body`);
      ...
      if (returnType !== 'None') {
        ...
        if (rawType) {
          emitter.line(`return types.${rawType}.model_validate(response.json())`);
        } else {
  ...
  private renderTypeRef(ref: IRTypeRef): string {
    ...
    if (ref.ref) t = `types.${ref.ref}`;
      ...
      if (ref.inline.kind === 'scalar') {
        if (ref.inline.scalar === 'integer' || ref.inline.scalar === 'number') t = 'float';
