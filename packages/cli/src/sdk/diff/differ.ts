...
export class SchemaDiffer {
  diff(oldSchema: IRSchema, newSchema: IRSchema): DiffResult {
    ...
    for (const [id, oldEp] of oldOps) {
      ...
      if (!newEp) {
        result.endpoints[id] = { type: 'removed', oldEndpoint: oldEp, changes: [] };
      } else {
    ...
    for (const [id, newEp] of newOps) {
      if (!oldOps.has(id)) {
        result.endpoints[id] = { type: 'added', newEndpoint: newEp, changes: [] };
      }
    }
  ...
  private diffEndpoints(oldEp: IREndpoint, newEp: IREndpoint): FieldChange[] {
    ...
    const diffParams = (oldParams: any[] = [], newParams: any[] = [], typeName: string) => {
      ...
      for (const [name, oldP] of oldMap) {
        ...
        if (!newP) {
          changes.push({ path: `${typeName}.${name}`, type: 'removed', oldValue: oldP });
        } else {
          if (JSON.stringify(oldP.type) !== JSON.stringify(newP.type)) {
            changes.push({ path: `${typeName}.${name}.type`, type: 'modified', oldValue: oldP.type, newValue: newP.type });
          }
          if (oldP.required !== newP.required) {
            changes.push({ path: `${typeName}.${name}.required`, type: 'modified', oldValue: oldP.required, newValue: newP.required });
          }
        }
      }
      ...
      for (const [name, newP] of newMap) {
        if (!oldMap.has(name)) {
          changes.push({ path: `${typeName}.${name}`, type: 'added', newValue: newP });
        }
      }
    };
    ...
    if (oldEp.requestBody && !newEp.requestBody) {
      changes.push({ path: 'requestBody', type: 'removed', oldValue: oldEp.requestBody });
    } else if (!oldEp.requestBody && newEp.requestBody) {
      changes.push({ path: 'requestBody', type: 'added', newValue: newEp.requestBody });
    } else if (oldEp.requestBody && newEp.requestBody) {
      if (JSON.stringify(oldEp.requestBody.type) !== JSON.stringify(newEp.requestBody.type)) {
        changes.push({ path: 'requestBody.type', type: 'modified', oldValue: oldEp.requestBody.type, newValue: newEp.requestBody.type });
      }
      if (oldEp.requestBody.required !== newEp.requestBody.required) {
        changes.push({ path: 'requestBody.required', type: 'modified', oldValue: oldEp.requestBody.required, newValue: newEp.requestBody.required });
      }
    }
  ...
  private diffTypes(oldType: IRType, newType: IRType): FieldChange[] {
    ...
    if (oldType.kind !== newType.kind) {
      changes.push({ path: 'kind', type: 'modified', oldValue: oldType.kind, newValue: newType.kind });
    ...
    if (oldType.kind === 'object' && newType.kind === 'object') {
      ...
      for (const [name, oldF] of oldFields) {
        ...
        if (!newF) {
          changes.push({ path: `fields.${name}`, type: 'removed', oldValue: oldF });
        } else {
          if (JSON.stringify(oldF.type) !== JSON.stringify(newF.type)) {
            changes.push({ path: `fields.${name}.type`, type: 'modified', oldValue: oldF.type, newValue: newF.type });
          }
          if (oldF.required !== newF.required) {
            changes.push({ path: `fields.${name}.required`, type: 'modified', oldValue: oldF.required, newValue: newF.required });
          }
        }
      }
      ...
      for (const [name, newF] of newFields) {
        if (!oldFields.has(name)) {
          changes.push({ path: `fields.${name}`, type: 'added', newValue: newF });
        }
      }
    }
