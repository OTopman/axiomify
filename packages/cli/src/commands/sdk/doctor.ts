...
export function registerSdkDoctorCommand(program: Command) {
  program
    .command('doctor')
    .description('Verify and diagnose local toolchains required for SDK targets')
    .action(async () => {
      console.log(pc.blue('ℹ Diagnosing local SDK target toolchains...\n'));

      const toolchains = [
        { name: 'TypeScript/JavaScript (Node.js)', cmd: 'node --version', req: 'Node.js' },
        { name: 'Python (pip/pydantic)', cmd: 'python3 --version', req: 'Python 3' },
        ...
        { name: 'Swift (swiftc/xcode)', cmd: 'swift --version', req: 'Swift / Xcode Command Line Tools' }
      ];
      ...
      for (const tc of toolchains) {
        try {
          const out = execSync(tc.cmd, { stdio: 'pipe' }).toString().trim().split('\n')[0];
