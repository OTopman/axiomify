...
export function registerSdkPublishCommand(program: Command) {
  program
    .command('publish')
    .description('Publish generated SDK packages to package registries (dry-run by default)')
    ...
    .action(async (options: { dryRun: boolean; registry?: string }) => {
      try {
        ...
        const targets = ['typescript', 'javascript', 'python', 'go', 'kotlin', 'swift', 'dart'];

        for (const target of targets) {
          console.log(pc.cyan(`  • Preparing publishing for target [${target}]`));
          
          let cmd = '';
          switch (target) {
            case 'typescript':
            case 'javascript':
              cmd = options.registry ? `npm publish --registry ${options.registry}` : 'npm publish';
              break;
            case 'python':
              cmd = 'python3 -m pip install --upgrade build twine && python3 -m build && python3 -m twine upload dist/*';
              break;
            case 'go':
          ...
          if (options.dryRun) {
            ...
            console.log(pc.yellow(`      [Action] Executing publishing command: ${cmd}`));
