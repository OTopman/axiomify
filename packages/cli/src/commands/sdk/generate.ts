...
export async function generateSdk(opts: GenerateOptions): Promise<boolean> {
  ...
  for (const t of opts.target) {
    ...
    try {
      ...
      if (opts.dryRun) {
        console.log(
          `  ${symbols.ok} [Dry Run] Would write ${files.length} files to ${path.relative(process.cwd(), outDir)}`,
        );
        ...
        console.log(
          `  ${symbols.ok} Wrote ${files.length} files to ${path.relative(process.cwd(), outDir)}`,
        );
      }
    } catch (err: any) {
