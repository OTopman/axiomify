...
export const ProfilerPanel: React.FC = () => {
  ...
  const handleClearProfiler = async () => {
    if (!confirm('Are you sure you want to clear all execution profiles?')) return;
    try {
  ...
  const filteredEntries = entries.filter((e) => {
    const matchesPath = e.request.path.toLowerCase().includes(searchPath.toLowerCase().trim());
    const matchesMethod = methodFilter === 'ALL' || e.request.method === methodFilter;
    
    let matchesStatus = true;
    if (statusFilter === 'success') {
      matchesStatus = !!e.response && e.response.status >= 200 && e.response.status < 400;
  ...
  const selectedRequest = entries.find((e) => e.requestId === selectedRequestId) || filteredEntries[0];
  ...
  const generateFlameBlocks = (req: ProfilerRequest) => {
    ...
    const sequentialItems = timeline.filter(item => sequentialTypes.includes(item.type));
    ...
    const itemDurationsAndOffsets = new Map<TimelineItem, { start: number; end: number }>();
    ...
    for (const serviceItem of serviceItems) {
      ...
      if (parentItem && itemDurationsAndOffsets.has(parentItem)) {
        ...
        const finalDuration = Math.min(serviceItem.duration, Math.max(0, parentMeta.end - nestedOffset - 0.1));
    ...
    const rootBlock = {
      ...
      level: 0,
      item: { name: `${req.request.method} ${req.request.path}`, type: 'request', duration: totalDuration }
    };
  ...
  const getBlockColor = (type: string) => {
    switch (type.toLowerCase()) {
      case 'request':
        return { fill: 'var(--bg-tertiary)', stroke: 'var(--border)', text: 'var(--text-primary)' };
      case 'hook':
        return { fill: 'rgba(108, 92, 231, 0.85)', stroke: 'rgb(108, 92, 231)', text: '#fff' };
      case 'middleware':
        return { fill: 'rgba(255, 159, 67, 0.85)', stroke: 'rgb(255, 159, 67)', text: '#fff' };
      case 'handler':
        return { fill: 'rgba(0, 210, 160, 0.85)', stroke: 'rgb(0, 210, 160)', text: '#fff' };
      case 'service':
        return { fill: 'rgba(0, 168, 204, 0.85)', stroke: 'rgb(0, 168, 204)', text: '#fff' };
      default:
        return { fill: 'var(--bg-tertiary)', stroke: 'var(--border)', text: 'var(--text-primary)' };
    }
  };
  ...
  const renderJsonView = (obj: any) => {
    if (!obj || Object.keys(obj).length === 0) return <div style={{ color: 'var(--text-muted)', fontStyle: 'italic', fontSize: '11px' }}>Empty</div>;
    return (
      <pre style={{ margin: 0, fontSize: '11px', fontFamily: 'var(--font-mono)', whiteSpace: 'pre-wrap', maxHeight: '180px', overflowY: 'auto', background: 'var(--bg-primary)', border: '1px solid var(--border)', padding: '8px', borderRadius: '4px' }}>
    ...
    <div>
      <div className="panel-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <div className="panel-subtitle">Analyze live request processing hook paths, middleware timing, state mutations, and DB latencies</div>
        </div>
        <button className="btn btn-secondary" onClick={handleClearProfiler} disabled={!sessionData}>
          Clear Profiler
        </button>
      </div>
      ...
      {entries.length === 0 ? (
        <div className="empty-state">
          ...
          <div className="empty-state-message">No execution traces captured. Hit the server with requests to populate the profiler.</div>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: '24px', minHeight: '650px', height: 'auto', textAlign: 'left' }}>
          ...
          <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <input
              ...
              style={{ width: '100%', margin: 0, padding: '6px 10px', fontSize: '12px', height: '32px' }}
            />
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              {(['ALL', 'success', 'error'] as const).map(s => (
                <button
                  ...
                  style={{ fontSize: '9px', padding: '3px 8px', cursor: 'pointer', flex: 1, textTransform: 'capitalize' }}
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '550px' }}>
              {filteredEntries.length === 0 ? (
                <div style={{ color: 'var(--text-muted)', fontSize: '11px', textAlign: 'center', padding: '12px' }}>No matching requests.</div>
              ) : (
                filteredEntries.map((e) => {
                  const isSelected = selectedRequest && e.requestId === selectedRequest.requestId;
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {selectedRequest ? (
              <>
                ...
                <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px' }}>
                      <button className="btn btn-secondary" onClick={handleZoomOut} style={{ padding: '3px 8px', fontSize: '12px', margin: 0 }}>Zoom -</button>
                      <button className="btn btn-secondary" onClick={handleZoomIn} style={{ padding: '3px 8px', fontSize: '12px', margin: 0 }}>Zoom +</button>
                      <button className="btn btn-secondary" onClick={handleZoomReset} style={{ padding: '3px 8px', fontSize: '12px', margin: 0 }}>Reset ↺</button>
                    </div>
                  </div>

                  {/* SVG interactive renderer */}
                  <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', overflow: 'hidden', cursor: isDragging.current ? 'grabbing' : 'grab', userSelect: 'none' }}>
                    <svg
                      width="100%"
                      height="200"
                    ...
                    >
                      <g transform={`translate(${panX}, 0) scale(${zoom}, 1)`}>
                        {generateFlameBlocks(selectedRequest).map((block, idx) => {
                          const totalDuration = selectedRequest.response?.durationMs || 1;
                          const widthPercent = (block.duration / totalDuration) * 980;
                          const xOffset = 10 + (block.start / totalDuration) * 980;
                          const yOffset = 10 + block.level * 50;
                          const height = 40;
                          const radius = 4;

                          const color = getBlockColor(block.type);
                          const isSelected = selectedBlock && selectedBlock.name === block.name && selectedBlock.type === block.type;
                          ...
                          return (
                            ...
                            >
                              <rect
                                ...
                                rx={radius}
                                ry={radius}
                                fill={isSelected ? 'var(--accent)' : color.fill}
                              ...
                              {showText && (
                                ...
                                >
                                  {block.name.length > (widthPercent * zoom) / 5
                                    ? block.name.slice(0, Math.floor((widthPercent * zoom) / 5)) + '..'
                                    : block.name}
                                </text>
                              )}
                              {/* Simple standard hover tooltip */}
                              <title>
                                {block.name}\nDuration: {block.duration.toFixed(2)} ms ({(block.duration / totalDuration * 100).toFixed(1)}%)
                              </title>
                            </g>
                          );
                        })}
                  ...
                  <div style={{ display: 'flex', gap: '16px', fontSize: '11px', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <div style={{ width: '12px', height: '12px', background: 'var(--bg-tertiary)', borderRadius: '2px', border: '1px solid var(--border)' }} />
                      <span>Request</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <div style={{ width: '12px', height: '12px', background: 'rgba(108, 92, 231, 0.85)', borderRadius: '2px' }} />
                      <span>Hook</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <div style={{ width: '12px', height: '12px', background: 'rgba(255, 159, 67, 0.85)', borderRadius: '2px' }} />
                      <span>Middleware</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <div style={{ width: '12px', height: '12px', background: 'rgba(0, 210, 160, 0.85)', borderRadius: '2px' }} />
                      <span>Handler</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <div style={{ width: '12px', height: '12px', background: 'rgba(0, 168, 204, 0.85)', borderRadius: '2px' }} />
                ...
                <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', padding: '24px' }}>
                  <div style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: '14px' }}>
                    Trace database query logs ({selectedRequest.queries?.length || 0})
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto', maxHeight: '200px' }}>
                    {!selectedRequest.queries || selectedRequest.queries.length === 0 ? (
                      <div style={{ fontStyle: 'italic', color: 'var(--text-muted)', fontSize: '11px', textAlign: 'center', padding: '12px' }}>
                        No queries captured during this execution.
                      </div>
                    ) : (
                      selectedRequest.queries.map((q, idx) => (
                        <div key={idx} style={{ background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '10px 14px' }}>
                          ...
                          <pre style={{ margin: 0, fontSize: '11px', fontFamily: 'var(--font-mono)', whiteSpace: 'pre-wrap', color: 'var(--text-primary)' }}>
                            {q.query}
                          </pre>
                        </div>
                      ))
              ...
              <div style={{ color: 'var(--text-muted)', textAlign: 'center', padding: '40px' }}>Select a request.</div>
            )}
          </div>
        </div>
      )}
      ...
      {selectedBlock && (
        <div style={{ position: 'fixed', right: 0, top: 0, bottom: 0, width: '420px', background: 'var(--bg-secondary)', borderLeft: '1px solid var(--border)', boxShadow: 'var(--shadow-lg)', zIndex: 1000, display: 'flex', flexDirection: 'column', animation: 'slideIn 0.2s ease-out' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--border)', background: 'var(--bg-primary)' }}>
            <div>
              <div style={{ fontSize: '9px', fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase' }}>
                {selectedBlock.type} details
              </div>
              <h3 style={{ margin: 0, fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)', wordBreak: 'break-all' }}>
                {selectedBlock.name}
              </h3>
            </div>
            <button className="btn btn-secondary" onClick={() => setSelectedBlock(null)} style={{ padding: '4px 8px', fontSize: '11px', margin: 0 }}>Close</button>
          </div>

          <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', background: 'var(--bg-primary)', padding: '12px', borderRadius: '4px', border: '1px solid var(--border)' }}>
              <div>
                <div style={{ fontSize: '9px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Duration</div>
                <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--success)', marginTop: '2px' }}>
                  {selectedBlock.duration.toFixed(2)} ms
                </div>
              </div>
              <div>
                <div style={{ fontSize: '9px', color: 'var(--text-muted)', textTransform: 'uppercase' }}>% of Request</div>
                <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--info)', marginTop: '2px' }}>
                  {selectedRequest?.response?.durationMs 
                    ? ((selectedBlock.duration / selectedRequest.response.durationMs) * 100).toFixed(1) + '%'
                    : '—'}
                </div>
              </div>
            </div>
            ...
            {selectedBlock.item?.before ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div>
                  <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Request Body (Before)</div>
                  {renderJsonView(selectedBlock.item.before.body)}
                </div>
                <div>
                  <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Request Body (After)</div>
                  {renderJsonView(selectedBlock.item.after.body)}
                </div>
                <div>
                  <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Route State (Before)</div>
                  {renderJsonView(selectedBlock.item.before.state)}
                </div>
                <div>
                  <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>Route State (After)</div>
                  {renderJsonView(selectedBlock.item.after.state)}
                </div>
              </div>
            ) : (
              <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontStyle: 'italic', textAlign: 'center', padding: '16px', border: '1px dashed var(--border)', borderRadius: '4px' }}>
