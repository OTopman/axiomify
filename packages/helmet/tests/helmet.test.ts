...
describe('useHelmet — HSTS object config and removePoweredBy:false', () => {
  it('configures HSTS with object options including preload', async () => {
    const app = new Axiomify();
    useHelmet(app, { hsts: { maxAge: 31536000, includeSubDomains: false, preload: true } });
    const req = { id: 'r', method: 'GET', url: '/', path: '/', ip: '127.0.0.1', headers: {}, body: undefined, query: {}, params: {}, state: {}, raw: {}, stream: null } as any;
    const headers: Record<string, string> = {};
    const res: any = { status: vi.fn().mockReturnThis(), header: vi.fn((k: string, v: string) => { headers[k] = v; return res; }), getHeader: vi.fn(), removeHeader: vi.fn(), send: vi.fn(), sendRaw: vi.fn(), error: vi.fn(), stream: vi.fn(), headersSent: false, statusCode: 200, raw: {}, capabilities: { sse: false, streaming: false } };
  ...
  it('when removePoweredBy is false, does not add X-Powered-By to removeHeaders', async () => {
    ...
    const req = { id: 'r', method: 'GET', url: '/', path: '/', ip: '127.0.0.1', headers: {}, body: undefined, query: {}, params: {}, state: {}, raw: {}, stream: null } as any;
    const res: any = { status: vi.fn().mockReturnThis(), header: vi.fn().mockReturnThis(), getHeader: vi.fn(), removeHeader: vi.fn(), send: vi.fn(), sendRaw: vi.fn(), error: vi.fn(), stream: vi.fn(), headersSent: false, statusCode: 200, raw: {}, capabilities: { sse: false, streaming: false } };
    for (const h of (app as any).hooks.hooks.onRequest) await h(req, res);
    const removedHeaders = (res.removeHeader as ReturnType<typeof vi.fn>).mock.calls.map((c: any[]) => c[0]);
