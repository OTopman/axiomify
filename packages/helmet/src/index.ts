...
export function useHelmet(app: Axiomify, options: HelmetOptions = {}): void {
  ...
  app.addHook('onRequest', (_req: AxiomifyRequest, res: AxiomifyResponse) => {
    ...
    setIfEnabled(res, 'X-Permitted-Cross-Domain-Policies', xPermittedCrossDomainPolicies, 'none');
    ...
    if (permissionsPolicy) res.header('Permissions-Policy', permissionsPolicy.trim());
