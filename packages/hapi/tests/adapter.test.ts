...
// ---------------------------------------------------------------------------
// Path conversion unit test
// ---------------------------------------------------------------------------

describe('toHapiPath (internal utility)', () => {
  // Access the internal function via a test-only route and inspect that Hapi
  // registered it correctly by making a real HTTP request.
  it('accepts :param path and routes correctly — Hapi converts internally', async () => {
    // Covered by integration test GET /users/:id below.
  });
});
...
describe('HapiAdapter: routing via Hapi router', () => {
  ...
  beforeAll(async () => {
    ...
    await native.start();
    port = native.info.port as number;
  });
  ...
  it('routes GET /users via Hapi router — returns 200', async () => {
    const res = await request(port, '/users', 'GET');
    expect(res.status).toBe(200);
    expect(res.data.data).toEqual([{ id: 1 }]);
  });

  it('routes GET /users/:id — Hapi populates req.params from {id} pattern', async () => {
    const res = await request(port, '/users/77', 'GET');
    expect(res.status).toBe(200);
    expect(res.data.data.id).toMatchObject({ id: '77' });
  });

  it('routes POST /echo — parses JSON body from stream', async () => {
    const res = await request(port, '/echo', 'POST', { hello: 'world' });
    expect(res.status).toBe(200);
    expect(res.data.data).toEqual({ hello: 'world' });
  });

  it('returns 404 for unregistered path via catch-all', async () => {
  ...
  it('strips constructor and prototype keys from the body', async () => {
...
describe('HapiAdapter: body size limit enforcement', () => {
  let adapter: HapiAdapter;
  let port: number;
