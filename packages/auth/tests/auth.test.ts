...
describe('Auth Plugin & Refresh', () => {
  ...
  it('returns 401 on wrong secret or expired token', async () => {
    ...
    const res1 = { status: vi.fn().mockReturnThis(), send: vi.fn(), headersSent: false, header: vi.fn().mockReturnThis() } as any;
    await app.handle(req1, res1);
    expect(res1.status).toHaveBeenCalledWith(401);

    const expiredToken = jwt.sign({ id: 1 }, secret, { expiresIn: '-1h' });
    const req2 = { method: 'GET', path: '/', headers: { authorization: `Bearer ${expiredToken}` }, id: '3', params: {} } as any;
    const res2 = { status: vi.fn().mockReturnThis(), send: vi.fn(), headersSent: false, header: vi.fn().mockReturnThis() } as any;
