...
describe('Auth — refresh handler', () => {
  ...
  const makeRes = () => ({ status: vi.fn().mockReturnThis(), send: vi.fn(), header: vi.fn().mockReturnThis(), headersSent: false });
  ...
  it('accepts token first use, then rejects after revoke', async () => {
    ...
    const handler = createRefreshHandler({ secret: accessSecret, refreshSecret, store, refreshTokenTtl: 60 });
    const jti = 'refresh-jti-1';
    await store.save(jti, 60);
    const token = jwt.sign({ id: 'u1', jti }, refreshSecret);

    const res1 = makeRes();
    await handler({ headers: { authorization: `Bearer ${token}` } } as any, res1 as any);
    expect(res1.status).toHaveBeenCalledWith(200);

    const res2 = makeRes();
    await handler({ headers: { authorization: `Bearer ${token}` } } as any, res2 as any);
  ...
  it('rejects refresh tokens without jti when store is configured', async () => {
    const store = new MemoryTokenStore();
    const handler = createRefreshHandler({ secret: accessSecret, refreshSecret, store });
    const token = jwt.sign({ id: 'u1' }, refreshSecret);
    const res = makeRes();
    await handler({ headers: { authorization: `Bearer ${token}` } } as any, res as any);
