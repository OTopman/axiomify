...
'@axiomify/http': major
'@axiomify/fastify': major
'@axiomify/express': major
'@axiomify/hapi': major
'@axiomify/auth': major
'@axiomify/cors': major
'@axiomify/helmet': major
'@axiomify/rate-limit': major
...
useGraphQL(app, { schema, context: (req) => ({ user: req.state.user }), maxDepth: 10 });
```
...
app.route({
  ...
  meta: { tags: ['Users'], description: 'Create a new user', summary: 'Create user' },
  handler: createUser,
});
```
