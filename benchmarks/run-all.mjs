  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      if (!ready) {
        child.kill('SIGKILL');
        reject(
          new Error(`Server ${serverDef.id} did not signal READY within 8s`),
        );
      }
    }, 8000);
    ...
    child.on('exit', (code, signal) => {
      if (!ready) {
        ...
        reject(
          new Error(
            `Server ${serverDef.id} exited (code=${code} signal=${signal}) before READY`,
          ),
        );
      }
    });
...
function printTable(results) {
  ...
  for (const r of results) {
    const group = r.id.startsWith('bare')
      ? 'BARE'
      : r.id.startsWith('axiomify-native')
      ? 'NATIVE'
      : 'AXIOMIFY';
...
async function main() {
  ...
  console.log(
    `Connections: ${CONNECTIONS}  Pipelining: ${PIPELINING}  Duration: ${BENCH_DURATION}s  Warmup: ${WARMUP_DURATION}s\n`,
  );
